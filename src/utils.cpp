...
fn bool parse_line(Arena *arena, i64 *pointer, Buf *file, ObjFile *obj) {

    i64 start = *pointer;
    bool file_ends = point_to_end_of_line(file, pointer);
    char *buf = (char*)file->buf;

    char *line = (char*)arena_push(arena, *pointer - start + 1 + 1);
    memcpy((void*)line, &buf[start], *pointer - start + 1);
    line[*pointer - start + 1] = '\0';
    ...
    file_ends = point_to_next_line(file, pointer);
...
fn LucyResult load_obj(Arena *arena, const char *file_path, ObjFile *out) {
    ...
    while(!is_end) {
        ...
        is_end = parse_line(arena, &pointer, &file, out);
    }
    arena_restore(arena, checkpoint);
